...
public:
    ...
    ~AnalyzeWindow();
    void createMainWidgetView();

    QString invokeCommand(QString cmdstr, QStringList arguments);

    int parseBusctlCallGetPropertiesJsonData(const QString jsonstr);

    BootTimes m_bootTime;

private:
    QWidget * m_pMainWidget;
    QVBoxLayout * m_vBoxLayout ;
    QLabel * m_systemdBootTimeLabelDesc;
    QLabel * m_systemdAnalyzePlotLabelDesc;

    QGraphicsScene * myGraphicScene;
    QSvgRenderer* myRenderer;
    CustomSvgGraphicsView * myCustomView;
    QGraphicsSvgItem * mySvgItem;

    CustomCanttChartView * canttTimeView;
